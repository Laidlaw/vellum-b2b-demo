...
// Smart image selector - guaranteed to return a working image URL
export function getProductImage(productName: string, category: string, productId: string): string {
  ...
  // Second priority: Category fallback image  
  if (CATEGORY_FALLBACK_IMAGES[category as keyof typeof CATEGORY_FALLBACK_IMAGES]) {
    return CATEGORY_FALLBACK_IMAGES[category as keyof typeof CATEGORY_FALLBACK_IMAGES];
  }

  // Third priority: Professional placeholder with product info
  const encodedName = encodeURIComponent(productName.slice(0, 20));
  return `https://via.placeholder.com/400x400/2c3e50/ffffff?text=${encodedName}`;
}
